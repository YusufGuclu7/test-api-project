      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell>Şirket Adı</TableCell> {/* Yeni Sütun Başlığı */}
          </TableRow>
        </TableHead>
        <TableBody>
          {records.map((rec) => (
            <React.Fragment key={rec.key}>
              <TableRow style={{ backgroundColor: '#f0f0f0' }}>
                ...
                <TableCell></TableCell> {/* Grup satırında boş bırakıyoruz */}
              </TableRow>
              {rec.items.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{item.rawData.hesap_adi || 'Bilinmiyor'}</TableCell> {/* Doğru anahtarı kullandık */}
                </TableRow>
              ))}
            </React.Fragment>
          ))}
